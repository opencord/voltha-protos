...
PROTOC    ?= docker run --rm --user $$(id -u):$$(id -g) -v $$PWD:/workdir voltha/voltha-tools:${VOLTHA_TOOLS_VERSION}-protoc protoc
PROTOC_SH ?= docker run --rm --user $$(id -u):$$(id -g) -v $$PWD:/go/src/github.com/opencord/voltha-protos/v3 --workdir=/go/src/github.com/opencord/voltha-protos/v3 voltha/voltha-tools:${VOLTHA_TOOLS_VERSION}-protoc sh
GO        ?= docker run --rm --user $$(id -u):$$(id -g) -v $$PWD:/workdir voltha/voltha-tools:${VOLTHA_TOOLS_VERSION}-golang go
